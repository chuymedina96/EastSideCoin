...
/* ---------------- RSA-OAEP (SHA-256 + fallback to SHA-1) ---------------- */
const oaepParams256 = {
...
export const encryptRSA = (dataB64, publicKeyPEM) => {
  ...
  const encrypted = publicKey.encrypt(raw, "RSA-OAEP", oaepParams256);
...
export const decryptRSA = (encryptedB64, privateKeyPEM) => {
  ...
  try {
    const out256 = privateKey.decrypt(encBytes, "RSA-OAEP", oaepParams256);
    return forge.util.encode64(out256);
  } catch (e) {
    // Legacy rows (if any) might be OAEP-SHA1
    const out1 = privateKey.decrypt(encBytes, "RSA-OAEP", oaepParams1);
    return forge.util.encode64(out1);
  }
};
